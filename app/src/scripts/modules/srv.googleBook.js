 ...
 */
app.service('googleBookService', function ($rootScope, $http, $q, $log, settingsService, base64, APP_CONFIG) {
    'use strict';
      ...
      if (searchQuery) {
        ...
        $http({
          ...
          if (usedCode) {
            ...
            $http({
              method: 'GET',
              url: 'https://www.googleapis.com/books/v1/volumes/?q=' + usedCode + '&projection=full&key=' + APP_CONFIG.googleApiKey,
              timeout: APP_CONFIG.timeout,
              headers: {
                'Access-Control-Allow-Origin': 'localhost',
                'Access-Control-Allow-Headers': 'Origin, X-Requested-With, Content-Type, Accept'
